...
success criterion (≥80% on real outline canonicals) belongs to Phase 4's fixture
...
# ---------------------------------------------------------------------------
# Phase 4 accuracy criterion — real outlines, deterministic flip-recovery.
#
# The fixture (backend/tests/fixtures/orientation_truth.json + the sibling
# outline CSVs) holds outlines extracted once, via SAM, from real photos — each
# at its PCA base angle (long axis horizontal). It runs without SAM (outlines are
# committed), so the test is fast and deterministic.
#
...
#      object. Those anatomical labels can't be trusted; the solidity filter in
#      the generator drops the worst, but the pipeline lesson stands.
#   2. The series spans very different specimen shapes. A single global EFA
#      reference tracks anatomical orientation only ~50% across such diversity —
#      inter-specimen shape variation swamps the 180° signal. Learned orientation
#      is strong *within* a morphologically coherent set, which is the real use
#      case.
#
